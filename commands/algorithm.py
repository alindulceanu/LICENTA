...
class signDetector:
    ...
    def findShapes(self, app, contArea, color):
        ...
        for i in range(len(app)):
            ...
            if len(app[i]) == 8 and contArea[i] > 1000 and color == "Red":
                signs.append(((x, y, w, h), "Stop"))

            elif len(app[i]) == 3 and contArea[i] > 1000 and color == "Red":
                signs.append(((x, y, w, h), "Yield"))

            elif len(app[i]) == 4 and contArea[i] > 1000 and color == "Blue":
                signs.append(((x, y, w, h), "Pedestrian"))

            elif len(app[i]) == 4 and contArea[i] > 1000 and color == "Yellow":
    ...
    def detectAndDraw(self, image):
        ...
        drawedImage = self.drawSigns(image, allSigns)
        return drawedImage
